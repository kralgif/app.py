...
║   AWS_ACCESS_KEY_ID   → AWS IAM User mit S3-Schreibrechten                  ║
║   AWS_SECRET_ACCESS_KEY → AWS IAM Secret Key                               ║
║   S3_BUCKET_NAME      → S3-Bucket-Name (Region eu-north-1, Stockholm)       ║
║                                                                              ║
╠══════════════════════════════════════════════════════════════════════════════╣
...
try:
    ...
    BOTO3_OK = False
    print("[S3] ⚠ boto3 fehlt – pip install boto3")
...
# ── AWS S3 (Audio-Cache, Region: eu-north-1 Stockholm) ───────────────────────
...
if BOTO3_OK:
    try:
        ...
        if S3_BUCKET:
            print(f"[S3] ✓ Client initialisiert – Bucket: {S3_BUCKET} (eu-north-1)")
        else:
            print("[S3] ⚠ S3_BUCKET_NAME nicht gesetzt – Caching deaktiviert")
    except Exception as _e:
        print(f"[S3] ⚠ Initialisierungsfehler: {_e}")
else:
    print("[S3] ⚠ boto3 nicht installiert – Caching deaktiviert")
...
# ══════════════════════════════════════════════════════════════════════════════
#  S3 AUDIO-CACHE  – ElevenLabs MP3 in Stockholm cachen
# ══════════════════════════════════════════════════════════════════════════════

def get_audio_url(text: str) -> Optional[str]:
    """
    Prüft ob ein MP3 für diesen Text bereits in S3 (eu-north-1) existiert.
    - JA  → S3-URL direkt zurückgeben (kein ElevenLabs-Aufruf nötig).
    - NEIN → ElevenLabs aufrufen (eleven_multilingual_v2), MP3 hochladen, URL zurückgeben.
    - Fehler → None zurückgeben (Fallback: generate_speech_base64 im Aufrufer).

    Dateiname = hashlib.md5(text.encode("utf-8")).hexdigest() + ".mp3"
    """
    if not s3_client or not S3_BUCKET or not text or not text.strip():
        return None

    # Dateiname aus MD5-Hash des Textes
    filename = hashlib.md5(text.encode("utf-8")).hexdigest() + ".mp3"
    s3_url   = f"https://{S3_BUCKET}.s3.eu-north-1.amazonaws.com/{filename}"

    # 1. Cache-Check: existiert die Datei bereits im Bucket?
    try:
        s3_client.head_object(Bucket=S3_BUCKET, Key=filename)
        print(f"[S3] ✓ Cache-Hit: {filename}")
        return s3_url
    except _S3Error as e:
        err_code = e.response.get("Error", {}).get("Code", "")
        if err_code not in ("404", "NoSuchKey"):
            print(f"[S3] ⚠ head_object Fehler ({err_code}): {e}")
            return None
        # 404 = noch nicht vorhanden → ElevenLabs aufrufen

    # 2. ElevenLabs: Audio generieren
    api_key  = os.environ.get("ELEVEN_API_KEY", "")
    voice_id = os.environ.get("ELEVEN_VOICE_ID", "21m00Tcm4TlvDq8ikWAM")

    if not api_key:
        print("[S3] ⚠ ELEVEN_API_KEY nicht gesetzt")
        return None
    if not REQUESTS_OK:
        print("[S3] ⚠ requests-Paket fehlt")
        return None
    ...
    try:
        ...
        resp = _requests.post(el_url, json=payload, headers=headers, timeout=15)

        if resp.status_code == 401:
            print("[S3/ElevenLabs] ⚠ API-Key ungültig")
            return None
        if resp.status_code == 429:
            print("[S3/ElevenLabs] ⚠ Rate-Limit")
            return None
        if resp.status_code != 200:
            print(f"[S3/ElevenLabs] ⚠ HTTP {resp.status_code}")
            return None

        audio_bytes = resp.content
        print(f"[S3/ElevenLabs] ✓ Audio generiert ({len(audio_bytes):,} Bytes)")

    except Exception as e:
        print(f"[S3/ElevenLabs] ⚠ API-Fehler: {e}")
        return None

    # 3. MP3 zu S3 hochladen
    try:
        ...
        print(f"[S3] ✓ Hochgeladen: {filename} → {s3_url}")
        return s3_url
...
def ask_ai():
    ...
    # ── TTS: S3-Cache → ElevenLabs → Base64-Fallback ────────────────────
    # Text natural aufbereiten (gleiche Logik wie generate_speech_base64)
    natural_text = NaturalTextProcessor.process(answer, emotion, lang)

    # Versuch 1: gecachte S3-URL (spart ElevenLabs API-Credits)
    audio_url = get_audio_url(natural_text)

    # Versuch 2: Base64-Fallback wenn S3 nicht konfiguriert / Fehler
    audio_b64 = None
    if not audio_url:
    ...
    return jsonify({
        "ok":        True,
        "answer":    answer,
        "audio_url": audio_url,   # S3-URL (bevorzugt, Browser streamt direkt)
        "audio_b64": audio_b64,   # Base64-Fallback (null wenn audio_url gesetzt)
        "lang":      lang,
    })
...
def status():
    ...
    return jsonify({
        ...
        "s3_cache":  bool(s3_client and S3_BUCKET),
...
if __name__ == "__main__":
    ...
    _s3_ok = bool(s3_client and S3_BUCKET)
    _s3_msg = ("OK  Bucket: " + S3_BUCKET + " (eu-north-1)") if _s3_ok else "deaktiviert (AWS_ACCESS_KEY_ID / S3_BUCKET_NAME setzen)"
    print(f"  S3 Audio-Cache: {_s3_msg}")
