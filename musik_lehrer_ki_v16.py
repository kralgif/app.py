...
║  API-KEYS (.env):  GROQ_API_KEY    = kostenlos auf console.groq.com        ║
║                    ELEVEN_API_KEY  = elevenlabs.io (10k Zeichen kostenlos)  ║
║                    ELEVEN_VOICE_ID = deine Stimmen-ID aus elevenlabs.io     ║
║  TTS-MODELL:       eleven_turbo_v2_5  (schnell, günstig, DE/EN/ES/FR)       ║
║  STARTEN:       python3 musik_lehrer_ki_v16.py → http://localhost:7878     ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  .env BEISPIEL (Datei namens '.env' im gleichen Ordner wie dieses Skript):  ║
║                                                                              ║
║    GROQ_API_KEY=gsk_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx                    ║
║    ELEVEN_API_KEY=sk_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx               ║
║    ELEVEN_VOICE_ID=21m00Tcm4TlvDq8ikWAM                                    ║
║    OPENAI_API_KEY=sk-proj-xxxx  (optional, nur als Fallback)                ║
║                                                                              ║
║  WICHTIG: .env NIEMALS in Git committen! In .gitignore eintragen:           ║
║    echo ".env" >> .gitignore                                                 ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
# ── .env laden (python-dotenv) ────────────────────────────────────────────
# Installieren: pip install python-dotenv
# .env Datei im gleichen Verzeichnis wie dieses Skript anlegen (siehe Kommentar unten)
try:
    from dotenv import load_dotenv as _load_dotenv
    _env_file = Path(__file__).parent / ".env"
    if _env_file.exists():
        _load_dotenv(dotenv_path=_env_file, override=False)
        print(f"[Config] ✓ .env geladen ({_env_file})")
    else:
        _load_dotenv(override=False)   # Fallback: .env im aktuellen Verzeichnis
        print("[Config] .env gesucht (nicht gefunden – Keys aus Umgebungsvariablen)")
    DOTENV_OK = True
except ImportError:
    DOTENV_OK = False
    print("[Config] ⚠ python-dotenv fehlt → pip install python-dotenv")
    print("[Config]   Fallback: Keys direkt aus Umgebungsvariablen (export KEY=...)")
...
GROQ_API_KEY   = os.environ.get("GROQ_API_KEY", "")    # wird aus .env geladen

# ── ElevenLabs TTS (10.000 Zeichen/Monat kostenlos) ──────────────────────
ELEVEN_API_KEY  = os.environ.get("ELEVEN_API_KEY", "")

# ELEVEN_VOICE_ID aus .env – falls nicht gesetzt: Fallback-Stimmen
# .env Beispiel:
#   ELEVEN_VOICE_ID=deine_voice_id_hier
#   ELEVEN_API_KEY=dein_api_key_hier
ELEVEN_VOICE_ID = os.environ.get("ELEVEN_VOICE_ID", "")          # Primär aus .env
ELEVEN_VOICE_DE = ELEVEN_VOICE_ID or "21m00Tcm4TlvDq8ikWAM"      # Fallback: Rachel (DE)
ELEVEN_VOICE_EN = ELEVEN_VOICE_ID or "EXAVITQu4vr4xnSDxMaL"      # Fallback: Bella (EN)
ELEVEN_MODEL    = "eleven_turbo_v2_5"   # Schnell + günstig, DE/EN/ES/FR unterstützt
...
class AIChat:
    ...
    def _speak_elevenlabs(self, text: str):
        ...
        def _run():
            try:
                if self._eleven_ok:
                    # Voice-ID Priorität: .env ELEVEN_VOICE_ID → sprachspezifischer Fallback
                    voice_id = (ELEVEN_VOICE_ID
                                or (ELEVEN_VOICE_DE if self.lang == "de" else ELEVEN_VOICE_EN))
        ...
        try:
            self.state={
                ...
                "system_status": {
                    ...
                    "dotenv_ok":     DOTENV_OK,
                    "voice_id_set":  bool(ELEVEN_VOICE_ID),
                    "scipy_ok":      SCIPY_OK,
                    "mp_ok":         MP_OK,
                },
...
async function runSystemCheck() {
  ...
  // TTS
  if(sys.tts_eleven) {
    const voiceInfo = sys.voice_id_set
      ? 'ELEVEN_VOICE_ID aus .env aktiv'
      : 'Fallback-Stimme (kein ELEVEN_VOICE_ID in .env)';
    checks.push(obCheckHTML('ok','🗣️','ElevenLabs TTS (eleven_turbo_v2_5)',
      voiceInfo + ' · Natürliche Stimme aktiv'));
  ...
  // .env Status
  if(sys.dotenv_ok) {
    checks.push(obCheckHTML('ok','📄','.env geladen',
      'python-dotenv aktiv – API-Keys sicher aus .env Datei'));
  } else {
    checks.push(obCheckHTML('warn','📄','.env nicht geladen',
      'pip install python-dotenv – oder Keys als Umgebungsvariablen setzen',
      'Mehr Info', "showErrToast('warn','.env Setup','Erstelle .env Datei im Skript-Verzeichnis mit ELEVEN_API_KEY=... und GROQ_API_KEY=...',false)"));
  }
