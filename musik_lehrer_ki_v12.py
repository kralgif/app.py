...
"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  🎸  MUSIK LEHRER KI v11.0  –  3D KI-Lehrerin LUNA  ·  Guitar & Piano  🎹  ║
║  v7 + Willkommens-Screen + garantierte Sprach-Begrüßung beim Öffnen       ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  NEU in v7.0:                                                               ║
║  ▸ Komplett neuer 3D-Charakter (bessere Geometrie, Mund-Morphing)          ║
║  ▸ Natürliche Bewegungen: Sakkaden, Doppel-Blinzeln, Phonem-Variation      ║
║  ▸ Web Speech Synthesis – Luna spricht LAUT mit Systemstimme               ║
║  ▸ Web Speech Recognition – Schüler kann Luna per Mikrofon ansprechen      ║
║  ▸ Mikrofon-Button im 3D-Fenster + Interim-Text-Anzeige                    ║
║  ▸ ALLE v6-Funktionen vollständig erhalten (109 Akkorde, Songs, Tutorials)  ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  INSTALLATION:  pip install opencv-python mediapipe numpy pyaudio scipy     ║
║  OPTIONAL:      pip install openai gtts                                     ║
║  STARTEN:       python3 musik_lehrer_ki_v7.py → http://localhost:7878       ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
HOLD_TIME      = 3.0
WRIST_LIMIT    = 32
CV_WINDOW      = "Musik Lehrer KI v12.0"
OPENAI_MODEL   = "gpt-4o"
...
LANG: Dict[str,Dict[str,str]] = {
    "de": {
        "welcome":       "Willkommen! Ich bin dein KI-Musiklehrer.",
    ...
    "en": {
        ...
        "posture_warn":  "Lower your wrist!",          "lesson_done":  "Lesson complete!",
...
class AudioAnalyzer:
    ...
    def _try_start(self):
        if not PYAUDIO_OK:
            print("[Audio] PyAudio fehlt – Simulation aktiv"); return
        try:
    ...
    def update(self, target_chord: Dict = None, target_pattern: str = "",
        ...
        self.onset = (delta > 0.015 and now - self._onset_cd > 0.05 and not self.is_silent)
        if self.onset: self._onset_cd = now
        self._prev_rms = rms*0.7 + self._prev_rms*0.3
...
class HandTracker:
    def __init__(self):
        ...
        self._no_hand=0;self._try_mp()

    def _try_mp(self):
        model=Path(__file__).parent/"hand_landmarker.task"
        if not MP_OK or not model.exists():
            print("[Tracker] Skin-Modus"); return
        try:
            ...
            self._mode="mediapipe";print("[Tracker] MediaPipe ✓")
        except Exception as e:
            print(f"[Tracker] {e} → Skin")
    ...
    def wrist_ang(self,hand,shape):
        ...
        v1=np.array(p[9])-np.array(p[0])
        v2=np.array(p[12])-np.array(p[9])
        n1,n2=np.linalg.norm(v1),np.linalg.norm(v2)
        if n1<1 or n2<1: return True,0.
        deg=math.degrees(math.acos(np.clip(np.dot(v1,v2)/(n1*n2),-1,1)))
        self._wrist.append(deg);avg=float(np.mean(self._wrist))
        return avg<WRIST_LIMIT,avg
...
# ══════════════════════════════════════════════════════════════════════════════
class AIChat:
    SYS=("Du bist ein freundlicher KI-Musiklehrer für Gitarre und Klavier. "
         "Beziehe dich auf erkannte Audio-Daten (Noten, Akkorde) wenn verfügbar. "
         "Antworte kurz (max 2-3 Sätze), konkret und motivierend.")

    def __init__(self,lang="de"):
        self.lang=lang;self._cli=None;self._hist=[]
        self.last="";self.thinking=False;self._spoken={}
        k=os.environ.get("OPENAI_API_KEY","")
        if OPENAI_OK and k:
            try: self._cli=_oai.OpenAI(api_key=k);print("[AI] OpenAI ✓")
            except: pass

    def ok(self): return self._cli is not None

    def ask(self,q,ctx=""):
        if not self.ok():
            self.last=LANG[self.lang].get("ai_unavail","KI nicht verfügbar");return
        self.thinking=True;self.last=LANG[self.lang].get("thinking","...")
        def _r():
            try:
                sys_p=self.SYS+(f"\nKontext: {ctx}" if ctx else "")
                self._hist.append({"role":"user","content":q})
                msgs=[{"role":"system","content":sys_p}]+self._hist[-8:]
                r=self._cli.chat.completions.create(model=OPENAI_MODEL,messages=msgs,
                    max_tokens=200,temperature=0.75)
                ans=r.choices[0].message.content.strip()
                self._hist.append({"role":"assistant","content":ans})
                self.last=ans;self.thinking=False;self._speak(ans)
            except Exception as e:
                self.last=f"Fehler: {e}";self.thinking=False
    ...
    def _speak(self,text):
        def _r():
            try:
                if self._cli:
                    r=self._cli.audio.speech.create(model="tts-1",voice="nova",
                        input=text,response_format="mp3")
                    r.stream_to_file("/tmp/ai_mlki.mp3")
                    _sub.Popen(["mpg123","-q","/tmp/ai_mlki.mp3"],
                               stdout=_sub.DEVNULL,stderr=_sub.DEVNULL)
                elif TTS_OK:
                    _gTTS(text=text,lang=self.lang[:2],slow=False).save("/tmp/ai_mlki.mp3")
                    _sub.Popen(["mpg123","-q","/tmp/ai_mlki.mp3"],
                               stdout=_sub.DEVNULL,stderr=_sub.DEVNULL)
            except: pass
        threading.Thread(target=_r,daemon=True).start()

    def speak_tip(self,text,cd=6.):
        now=time.time();k=text[:30]
        if now-self._spoken.get(k,0)>cd:
            self._spoken[k]=now;self._speak(text)
...
class ChordTrainer:
    ...
    def _do_master(self,fb,ai):
        ...
        ai.speak_tip(fb.t("chord_ok",xp=xp))
...
class Instructor:
    def __init__(self,profile,lang="de"):
        ...
        self.ai.speak_tip(self.fb.t("welcome"))
    ...
    def process(self,frame):
        ...
        if hands:
            ...
            if self._posture_t>=2.:
                self._posture_t=0
                self._wrist_ok,self._wrist_ang=self.tracker.wrist_ang(h0,frame.shape)
                if not self._wrist_ok:
                    self.fb.push(self.t("posture_warn"),OR["danger"],3.5)
                    self.ai.speak_tip(self.t("posture_warn"))
        ...
        if leveled:
            ...
            if did:
                ...
                self.ai.speak_tip(self.t("level_up",lvl=self.profile.level))
    ...
    def _update_state(self,acc):
        try:
            self.state={
                ...
                "posture":{"wrist_ok":self._wrist_ok,"angle":round(self._wrist_ang,1)},
                ...
                "songs_available": list(SONGS.get(self.profile.instrument,{}).keys()),
            }
        except Exception as e:
            print(f"[State] {e}")
    ...
    "guitar": [
        ...
        # ── LEVEL 1  ───────────────────────────────────────────────────────────
        {
            ...
            "technique": {
                ...
                "luna_speech": "Hallo! Ich bin Luna, deine KI-Musiklehrerin! Heute fangen wir ganz am Anfang an. Nimm deine Gitarre und schau dir jeden Teil genau an – das Fundament für alles was folgt!",
            },
            "exercise": {
            ...
            "chords": [],
            "speech": {"de": "Hallo! Ich bin Luna. Heute beginnen wir ganz von vorne.", "en": "Hello! I'm Luna. Today we start from scratch."},
        },
...
class CurriculumManager:
    ...
    def get_lessons(self, instrument: str) -> List[Dict]:
    ...
    def recommended_idx(self, profile: 'Profile') -> int:
        for i, l in enumerate(self.get_lessons(profile.instrument)):
            if i >= profile.lesson_idx and l['level'] <= profile.level:
                return i
        return min(profile.lesson_idx, len(self.get_lessons(profile.instrument))-1)

    def total_xp(self, instrument: str) -> int:
        return sum(l.get('xp_reward', 100) for l in self.get_lessons(instrument))
...
def _build_html(state: Dict) -> str:
    """3D KI-Lehrer Luna – vollständige Web-UI."""
    return """<!DOCTYPE html>
<html lang="de">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Musik Lehrer KI v9.0 – Luna</title>
...
<!-- ═══ WILLKOMMENS-OVERLAY ═══════════════════════════════════════════════ -->
<div id="wov" onclick="_doUnlockAndGreet();_closeOverlay()">
  <div class="wov-rings">
    <div class="wov-ring"></div>
    <div class="wov-ring"></div>
    <div class="wov-ring"></div>
  </div>
  <div class="wov-avatar">🎸</div>
  <h2>Hallo! Ich bin Luna</h2>
  <p class="wov-sub-title">
    Deine persönliche <strong>KI-Musiklehrerin</strong><br>
    Ich spreche laut mit dir · Analysiere dein Spiel · Beantworte deine Fragen
  </p>
  <button id="wov-btn" onclick="_doUnlockAndGreet();_closeOverlay()">
    ▶ &nbsp; Starten &amp; Ton einschalten
  </button>
  <div id="wov-hint">Klick zum Aktivieren &nbsp;·&nbsp; Chrome · Edge · Firefox · Safari</div>
</div>
...
<header>
  <div class="hdr-logo">
    ...
    <span class="hdr-logo-text">MUSIK LEHRER KI v11.0</span>
  </div>
...
<div class="panel">
  ...
  <div id="chat-box">
    ...
    <div class="chat-input-row">
      <input id="chat-input" placeholder="Frag Luna…" onkeydown="if(event.key==='Enter')sendChat()">
...
<div id="center">
  ...
  <button id="mic-button" title="Sprich mit Luna" onclick="toggleMic()">🎤</button>
...
<div id="avatar-panel">
  ...
  <div id="av-infobar">
    <div class="av-name">Luna · KI-Avatar</div>
...
<div class="panel right">
  ...
  <div class="panel-inner">
    ...
    <div id="rtab-audio">
      ...
      <div id="rp-onset" style="display:none;background:#0E3020;border-radius:6px;padding:3px 10px;font-size:.68rem;color:var(--green);margin:3px 0;border:1px solid #2EFF9A30">● ANSCHLAG</div>
    </div>
...
<script>
// ════════════════════════════════════════════════════════════════════════════
//  3D KI-LEHRER "LUNA" – Three.js prozedural
// ════════════════════════════════════════════════════════════════════════════
(function() {
...
// ════════════════════════════════════════════════════════════════════════════
// ════════════════════════════════════════════════════════════════════════════
//  LUNA CHARAKTER-AUFBAU v7 – natürlichere Geometrie + Mund-Morphing
// ════════════════════════════════════════════════════════════════════════════
...
function pickPreferredVoice() {
  ...
  const order = speechLang === 'de-DE'
    ? ['Google Deutsch','Marlene','Katja','Hedda','Sabine','Anna','Helena',
       'de-DE','de_DE','de-AT','de-CH']
    : ['Google US English','Samantha','Karen','Moira','Victoria',
  ...
  silent.onend = ()=>{
    ...
    setTimeout(()=>{
      const text = _pendingGreeting || (speechLang.startsWith('de')
        ? 'Hallo! Ich bin Luna, deine persönliche KI-Musiklehrerin! Lass uns gemeinsam Musik machen!'
...
function initSpeechRecognition() {
  ...
  recognition.onstart = ()=>{
    ...
    if(btn){ btn.classList.add('active'); btn.title='Luna hört zu – klick zum Stoppen'; }
    const hdr = document.getElementById('hdr-mic-status');
    if(hdr){ hdr.textContent='🎤 Hört…'; hdr.style.display=''; }
  };
  recognition.onend = ()=>{
    micActive = false;
    const btn = document.getElementById('mic-button');
    if(btn){ btn.classList.remove('active'); btn.title='Sprich mit Luna'; }
    const hdr = document.getElementById('hdr-mic-status');
  ...
  recognition.onresult = (ev)=>{
    ...
    if(intEl && interim){ intEl.textContent='🎤 "'+interim+'"'; intEl.style.display='block'; }
    if(final.trim()){
      if(intEl) intEl.style.display='none';
      const inp = document.getElementById('chat-input');
      if(inp) inp.value = final.trim();
      addChat('Du', final.trim());
      sendAIQuestion(final.trim());
    }
  };
  recognition.onerror = (e)=>{
    console.warn('[Mic]', e.error); micActive = false;
    if(e.error==='not-allowed')
      lunaSay('Bitte erlaube Mikrofon-Zugriff im Browser!','correcting');
  };
}
...
function toggleMic() {
  if(!recognition){
    lunaSay('Dein Browser unterstützt leider keine Sprach-Erkennung. Bitte tippe deine Frage.','explaining');
    return;
  }
...
function setLTab(t) {
  activeTab=t;
  ['lesson','chords','song'].forEach(id=>{
    const el=document.getElementById('ltab-'+id);
    if(el) el.style.display=id===t?'':'none';
  });
  document.querySelectorAll('.panel:first-child .tab').forEach((el,i)=>{
    el.classList.toggle('active',['lesson','chords','song'][i]===t);
  });
}
function setRTab(t) {
  activeRTab=t;
...
function updateUI(s) {
  ...
  setAnalysisScore('an-clean', an_cln*100, '%');
...
function renderSongList(s) {
  ...
  document.getElementById('song-list-left').innerHTML=songs.map((sk,i)=>`
    <div onclick="startSong('${sk.replace(/'/g,"\\'")}')" style="background:#1A1A35;border:1px solid var(--border);
      border-radius:8px;padding:8px 10px;margin-bottom:5px;cursor:pointer;font-size:.78rem;
      transition:.15s" onmouseover="this.style.borderColor='var(--song)'" onmouseout="this.style.borderColor='var(--border)'">
      <div style="font-weight:700;color:var(--coal)">♪ ${sk}</div>
    </div>`).join('');
}
...
function renderLessonList(curriculum) {
  ...
  el.innerHTML=curriculum.map((les,i)=>`
    <div class="lesson-card ${i===currentLessonIdx?'active':''} ${i<currentLessonIdx?'done':''}"
      onclick="selectLesson(${i})">
      <div style="display:flex;justify-content:space-between;align-items:center">
        <div class="lesson-level">Lv.${les.level}</div>
        <span style="font-size:.58rem;color:var(--yellow);font-family:var(--mono)">+${les.xp_reward||100} XP</span>
      </div>
      <div class="lesson-title">${i===currentLessonIdx?'▶ ':''}${les.title}</div>
...
async function sendChat() {
  ...
  try {
    ...
    const ctx=`Instrument:${STATE?.chord?.data?'guitar':'piano'},Akkord:${ch.key||'?'},Note:${au.note_name||'?'},Hz:${(au.pitch_hz||0).toFixed(0)},PolyScore:${((au.poly?.chord_score||0)*100).toFixed(0)}%`;
...
// ════════════════════════════════════════════════════════════════════════════
//  INIT v8 – Willkommens-Overlay + Audio-Unlock
// ════════════════════════════════════════════════════════════════════════════
...
(async()=>{
  const initR = await fetch('/state');
  const initS = await initR.json();
  STATE       = initS;
  LessonData  = initS.teacher?.curriculum;
  const lang  = initS.lang || 'de';
  speechLang  = lang==='de' ? 'de-DE' : 'en-US';
  pickPreferredVoice();
  renderLessonList(LessonData);

  // Begrüßungstext vorhalten – wird abgespielt sobald der User klickt
  _pendingGreeting = lang==='de'
    ? 'Hallo! Ich bin Luna, deine persönliche KI-Musiklehrerin! Ich analysiere in Echtzeit deine Haltung, deinen Rhythmus und wie sauber du spielst. Du kannst mich auch direkt ansprechen – drücke den Mikrofon-Knopf oder schreibe mir unten im Chat. Lass uns gemeinsam Musik machen!'
    : 'Hello! I am Luna, your personal AI music teacher! I analyse your posture, rhythm, and sound quality in real time. You can also talk to me directly – press the microphone button or write in the chat. Let us make music together!';
})();
        ...
        if p == "/select_chord" and _instructor:
            ...
            try:
                ...
                if not ctx:
                    ...
                    ctx = (f"Instrument:{_instructor.profile.instrument},"
...
def terminal_setup_v6(pm):
    print("\n╔═══════════════════════════════════════════════════════════════════════╗")
    print("║  🎸  MUSIK LEHRER KI v6.0  ·  3D KI-LEHRER LUNA  ·  Orange UI  🎹  ║")
    print("║  Polyphon · Strumming · Song-Modus · 3D-Lehrer · 109 Akkorde · Web   ║")
    print("╚═══════════════════════════════════════════════════════════════════════╝\n")
...
def main():
    ...
    print(f"\n✓ Willkommen, {profile.name}! Luna ist bereit.")
    ...
    for idx in range(4):
        ...
        if c.isOpened(): cap = c; print(f"✓ Kamera {idx}"); break
        c.release()

    _instructor = InstructorV6(profile, lang)

    if not cap:
        print("❌ Keine Kamera – nur Web-UI verfügbar.")
        ph = np.ones((720,1280,3), np.uint8)*20
        _txt(ph,"Musik Lehrer KI v6.0 – Luna",(60,360),1.0,OR["orange"])
        _txt(ph,"http://localhost:"+str(WEB_PORT),(60,410),0.7,OR["muted"])
